...
<script>
import { getStaffInfo, getStaffBasic, getJobsInfo } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getJobDetail() {
      const jobInfo = await getJobsInfo(this.userId)
      const userInfo = await getStaffInfo(this.userId)// 获取个人基本信息
      this.formData = { ...userInfo, ...jobInfo }
    },
    async getPersonalDetail() {
      const userInfo = await getUserInfo(this.userId)
      const StaffInfo = await getStaffBasic(this.userId) // 获取个人基本信息
      this.formData = { ...userInfo, ...StaffInfo }
    }
  }
}
</script>
