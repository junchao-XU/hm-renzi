    ...
    <div v-loading="loading" class="historicalTable">
      <div v-for="( itemes, index) in tableData" :key="index" class="itemes">
        <div class="itemTopLab" :class="{act: itemes.act}">
          ...
          <div>
            <p class="title">{{ itemes.month }}社保报表 <span>{{ itemes.creationTime | filterTime('YYYY年MM月DD日') }}</span></p>
...
export default {
  ...
  methods: {
    // 入职时间处理
    transformDateFormat(row, column, cellValue, index) {
      return filterTime(cellValue, 'YYYY年MM月DD日')
    },
    // 查询社保历史归档列表
    async getArchivingList() {
      ...
      this.loading = false
      if (this.tableData.length === 0) return this.$message.warning('今年还没有历史归档哦!')
    },
    // 打开报表发请求  获取月份数据报表
